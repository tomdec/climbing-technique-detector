...
def plot_confidence_distributions(
    ...
    def extract_confidence_arr(df: DataFrame) -> List[float]:
        arr: List[PredictedKeyPoint] = df.map(lambda x: x.predicted_landmark).values.reshape(-1)
        return [prediction.visibility for prediction in arr if not prediction.is_missing()]

    arrs = [extract_confidence_arr(df) for df in data]

    if combine_plots:
        plot_histograms(names, arrs, save_location, 
...
def plot_landmark_confidence_distributions(
    ...
    def extract_confidence_arr(included_cols: List[str]) -> List[float]:
        ...
        arr: List[PredictedKeyPoint] = selection.map(lambda x: x.predicted_landmark).values.reshape(-1)
        return [prediction.visibility for prediction in arr if not prediction.is_missing()]

    arrs = [extract_confidence_arr(col_set) for col_set in columns]

    plot_histogram_grid(names=names, 
        data=arrs, 
        grid=(len(names), 1),
        save_location=save_location,
        title="Confidence distribution",
...
def plot_AP_per_landmark(
    ...
    if save_location: __save_current_figure(save_location)