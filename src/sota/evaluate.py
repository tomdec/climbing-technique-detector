...
def evaluate_with_majority_voting(
    image: MatLike, model: SOTA, window: List[str], window_size: int = 5
) -> str:
...
def collect_evaluation_performance(video_path: str, model: SOTA) -> DataFrame:
    ...
    try:
        for valid_segment in valids:
            ...
            while vid_capture.isOpened() and frame_num < stop_frame:
                ...
                t0_cpu = process_time()
                t0_seq = perf_counter()
                prediction = evaluate_with_majority_voting(
                    image, model, window, window_size
                )
                t1_cpu = process_time()
                t1_seq = perf_counter()
    ...
    results = DataFrame(
        data=zip(
            frame, labels_arr, original, processed, total_speed_cpu, total_speed_seq
        ),
        columns=[
            "frame",
            "labels",
