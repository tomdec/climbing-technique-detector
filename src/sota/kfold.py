...
from src.sota.model import (
    SOTA,
    SOTAConstructorArgs,
    SOTAModelInitializeArgs,
    SOTAMultiRunTrainArgs,
)
...
class SOTAFoldCrossValidation(AbstractFoldCrossValidation):
    ...
    @staticmethod
    def evaluation_instance(name: str, model_arch: str) -> "SOTAFoldCrossValidation":
        ...
        """
        return SOTAFoldCrossValidation(
            model_args=SOTAConstructorArgs(
                name=name,
                model_initialize_args=SOTAModelInitializeArgs(model_arch=model_arch),
            )
        )
    ...
    def __get_fold_dataset_path(self):
        return join(
            self._model_args.data_root_path, "img", self._model_args.dataset_name
        )
    ...
    def build_fold(self, fold_num, train, val, test, full_data):
        ...
        print(
            f"Fold {fold_num}: Train size = {len(train)}, Val size = {len(val)}, "
    ...
    @override
    def train_folds(self, train_run_args: SOTAMultiRunTrainArgs | None = None):
