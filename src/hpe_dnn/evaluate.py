...
def _get_input_features(
    image: MatLike, hpe_tool: Holistic, imputer: SimpleImputer
) -> dict:
    ...
    if landmarks.isnull().any().any():
        landmarks = DataFrame(
            imputer.fit_transform(landmarks), columns=landmarks.columns
        )
    landmarks = {
        key: value.to_numpy()[:, tf.newaxis] for key, value in landmarks.items()
    }
    return landmarks
...
def evaluate(
    ...
    binarizer: LabelBinarizer | None = None,
) -> str:
...
def collect_evaluation_performance(video_path: str, model: HpeDnn) -> DataFrame:
    ...
    imputer = SimpleImputer(
        missing_values=None, strategy="constant", fill_value=0, keep_empty_features=True
    )
...
def print_results(evaluation_root: str):
    ...
    print_original_results(results)
    print_common_results(results)
    print_hpe_inference_comparison(results, "DNN")
