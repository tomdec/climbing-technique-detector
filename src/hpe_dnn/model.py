...
class HpeDnnConstructorArgs(ModelConstructorArgs):
    ...
    @override
    def copy_with(self, name=None, dataset_name=None) -> "HpeDnnConstructorArgs":
        return HpeDnnConstructorArgs(
...
class HpeDnnTrainArgs(TrainArgs):
    ...
    def __init__(self, epochs=20, balanced=False, additional_config={}, augment=False):
        ...
        if balanced and not augment:
            print(
                "Warning: avoid reusing the exact same image multiple times by also enabling "
                + "augmentation when balancing the dataset"
            )
...
class HpeDnnMultiRunTrainArgs(MultiRunTrainArgs):
    ...
    def __init__(
        ...
        MultiRunTrainArgs.__init__(self, runs, train_args)
...
class HpeDnn(ClassificationModel):
    ...
    def train_model(self, args: HpeDnnTrainArgs):
        ...
        train_ds = self.__get_data_from_split(
            "train", augment=args.augment, balance=args.balanced, shuffle=True
        )
        val_ds = self.__get_data_from_split(
            "val", augment=False, balance=False, shuffle=False
        )
    ...
    def _fresh_model(self):
        ...
        train_ds = self.__get_data_from_split(
            "train", augment=False, balance=False, shuffle=False
        )
        debugging = False
        model_func = get_model_factory(self.model_arch)
        self.model = model_func(
            train_ds,
            self.model_initialize_args.normalize,
            debugging,
            self.model_initialize_args.dropout_rate,
        )
    ...
    def test_model(self, args: TestArgs):
        ...
        test_data = self.__get_data_from_split(
            split="test", augment=False, balance=False, shuffle=False
        )
    ...
    def __get_data_from_split(
        self, split: str, augment, balance, shuffle
    ) -> tf.data.Dataset:
