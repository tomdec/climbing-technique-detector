...
class HpeDnnFoldCrossValidation(AbstractFoldCrossValidation):
    ...
    @override
    @property
    def model_constructor_args(self) -> HpeDnnConstructorArgs:
    ...
    def __get_fold_dataset_path(self):
        return join(
            self._model_args.data_root_path, "df", self._model_args.dataset_name
        )

    @override
    def _get_additional_config(self, context_config: Dict = {}) -> Dict:
        return super()._get_additional_config(context_config) | {
            "dropout_rate": self.model_constructor_args.model_initialize_args.dropout_rate
        }
    ...
    def build_fold(self, fold_num, train, val, test, full_data: DataFrame):
        ...
        print(
            f"Fold {fold_num}: Train size = {len(train)}, Val size = {len(val)}, Test size = {len(test)}"
        )
    ...
    @override
    def train_folds(self, train_run_args: HpeDnnMultiRunTrainArgs | None = None):
