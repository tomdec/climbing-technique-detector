...
def _swap_to_full_dataset(model_args: ModelConstructorArgs) -> ModelConstructorArgs:
    ...
    new_name = model_args.dataset_name.replace("_kf", "_full_kf")
...
class AbstractFoldCrossValidation:
    ...
    def __init__(
        self,
        model_args: ModelConstructorArgs,
    ...
    def get_full_data_list(self) -> Any:
        raise_not_implemented_error(
            self.__class__.__name__, self.get_full_data_list.__name__
        )
    ...
    def print_box_plot(self):
        raise_not_implemented_error(
            self.__class__.__name__, self.print_box_plot.__name__
        )
    ...
    def __resolve_train_args(self, train_run_args: MultiRunTrainArgs | None = None):
        ...
        if self.train_run_args is None:
            raise (
                ArgumentError(
                    "Provide 'train_run_args' argument in either the constructor or "
                    "'train_folds' method to execute training."
                )
            )

    def __train_fold(
        self, fold_num: int, full_data: Any, train: ndarray, test: ndarray
    ):
        ...
        model.test_model(
            args=TestArgs(write_to_wandb=True, additional_config=additional_config)
        )

        self.clear_fold()

    def train_folds(self, train_run_args: MultiRunTrainArgs | None = None):
    ...
    def train_single_fold(
        self, fold_num: int, train_run_args: MultiRunTrainArgs | None = None
    ):
        if fold_num > self._kf.get_n_splits():
            raise Exception(
                f"Fold number {fold_num} is too high, model is configured with "
    ...
    def test_folds_on_full(self):
        ...
        try:
            ...
            for i, (_, test) in enumerate(self._kf.split(full_data)):
                ...
                additional_config = self._get_additional_config(
                    context_config={"fold": fold_num, "on_full": True}
                )
                model.test_model(
                    args=TestArgs(
