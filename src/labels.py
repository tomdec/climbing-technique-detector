...
def get_labels_as_dataframe(label_path) -> LabelsCSV:
    return read_csv(
        label_path, header=None, names=["start", "stop", "label", "cvs_start"]
    )
...
def find_valid_segments(label_path: str) -> List[Tuple[int, int]]:
    ...
    valid_segments.append(current_segment)
    print(
        f"Reduced {labels.index.size} individual segments to {len(valid_segments)} continuous "
        + "valid segments."
    )
