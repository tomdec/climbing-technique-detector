...
def __add_base_name(run: Run) -> dict:
    ...
    return {
        "base_name": run.name[:idx] if idx != -1 else run.name
    }
...
__unset_on_full_filter: dict = {"$and": [
    {"config.on_full": { "$ne": True }},
    {"config.on_full": { "$ne": False }}
]}

def update_run_config(filters: dict = {},
        config_patcher: Callable[[Run], dict] = __no_update):
    api = PublicApi()
    entity = api.default_entity
    project = "detect-climbing-technique"

    runs: List[Run] = api.runs(f"{entity}/{project}", filters)
